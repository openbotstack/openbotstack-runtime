...
// Execute implements ToolRunner. It creates a ToolContext and delegates
// to Invoke, wrapping the result in a StepResult.
func (c *RegistryClient) Execute(ctx context.Context, name string, input map[string]any, ec *execution.ExecutionContext) (*execution.StepResult, error) {
	tc := NewToolContext(ctx, ec)

	output, err := c.Invoke(tc, name, input)
